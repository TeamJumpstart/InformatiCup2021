    ...
    def act(self, cells, player, opponents, round):
        def sum_future_steps(pos, direction, n_steps, steps=[]):
            ...
            if n_steps <= 0:
                ...
                number_of_moves += sum_future_steps(pos, direction.turn_left(), n_steps - 1, steps)
                # try to move forward, add num of possible future steps
                number_of_moves += sum_future_steps(pos, direction, n_steps - 1, steps)
                # try to move right, add num of possible future steps
                number_of_moves += sum_future_steps(pos, direction.turn_right(), n_steps - 1, steps)
